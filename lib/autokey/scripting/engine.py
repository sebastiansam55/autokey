...
Folders created within temporary folders must themselves be set temporary")
    ...
    def create_phrase(self, folder, name: str, contents: str,
        ...
        C{engine.create_phrase(folder, "My new Phrase", "This is the Phrase content", abbreviations=["abc", "def"], \
        ...
        @param abbreviations: May be a single string or a list of strings. Each given string is assigned as an abbreviation \
        to the newly created phrase.

        @param hotkey: The hotkey parameter accepts a 2-tuple, consisting of a list of modifier keys in the first \
        element and an unshifted (lowercase) key as the second element.
            ...
            values permitted:
                C{<ctrl>}, C{<alt>}, C{<super>}, C{<hyper>}, C{<meta>}, C{<shift>}
        ...
        @param send_mode: This parameter configures how AutoKey sends the phrase content, for example by typing or by pasting \
        using the clipboard. It accepts items from the SendMode enumeration, which is also available from the engine \
        object as engine.SendMode. The parameter defaults to engine.SendMode.KEYBOARD.

            Available send modes are:
                C{KEYBOARD}, C{CB_CTRL_V}, C{CB_CTRL_SHIFT_V}, C{CB_SHIFT_INSERT}, C{SELECTION}

            To paste the Phrase using C{<shift>+<insert>}, set C{send_mode=engine.SendMode.CB_SHIFT_INSERT}

        @param window_filter: Accepts a string which will be used as a regular expression to match window titles or \
