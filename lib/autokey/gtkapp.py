...
class Application:
    ...
    def __init__(self):
        ...
        if len(missing_programs)>0 or len(missing_modules)>0:
            ...
            self.show_error_dialog("AutoKey Requires the following programs/python modules to be installed to function properly", error_message)
    ...
    def path_removed(self, path):
        ...
        if self.monitor.has_watch(path) and os.path.isdir(path):
            self.monitor.suspend()
